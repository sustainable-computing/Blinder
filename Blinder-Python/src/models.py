...
def maml_update(model, lr, user_aux_weight_bias=None, grads=None):
    """
    ...
    **Example**
    ...
    if grads is not None:
        ...
        for p, g in zip(params, grads):
            if g is not None:
                if user_aux_weight_bias is not None:
                    grad_bias = user_aux_weight_bias[counter]
                    # g_ratio = torch.sum(g) / (torch.sum(g)+torch.sum(grad_bias)+1e-12)
                    # p.update = - lr * (g * (1-g_ratio) + grad_bias*g_ratio) # modified to normalize, divide by aux_ep
                    # p.update = - lr * (g + grad_bias*0.5) # modified to normalize, divide by aux_ep
                    p.update = - lr * (g + grad_bias)/2 # modified to normalize, divide by aux_ep
    ...
    **Example**
    ...
    def adapt(self,
        ...
        * **allow_nograd** (bool, *optional*, default=None) - Whether to allow adaptation with
        ...
        # Update the module
        self.module = maml_update(self.module, self.lr, user_aux_weight_bias, gradients)
        # self.module = maml_update(self.module, self.lr, counter, gradients)
